<?php
/**
 * Server-side rendering for `zenyx/hero-05`.
 *
...
$raw_nav = isset( $attributes['navItems'] ) && is_array( $attributes['navItems'] ) ? $attributes['navItems'] : array();
$nav_items = array();
foreach ( $raw_nav as $i => $item ) {
	if ( ! is_array( $item ) ) {
		continue;
	}
	...
	if ( '' === $label ) {
		$label = isset( $default_nav[ $i ]['label'] ) ? (string) $default_nav[ $i ]['label'] : '';
	}
	$nav_items[] = array(
		'label' => $label,
		'url'   => $url,
...
$aside_highlight = isset( $attributes['asideHighlight'] ) ? (string) $attributes['asideHighlight'] : '';
...
$raw_platforms = isset( $attributes['platformLinks'] ) && is_array( $attributes['platformLinks'] ) ? $attributes['platformLinks'] : array();
$platforms     = array();
foreach ( $raw_platforms as $index => $item ) {
	if ( ! is_array( $item ) ) {
		continue;
	}
	$def     = isset( $default_platforms[ $index ] ) ? $default_platforms[ $index ] : $default_platforms[0];
	$label   = isset( $item['label'] ) ? trim( (string) $item['label'] ) : (string) $def['label'];
	$url     = isset( $item['url'] ) ? (string) $item['url'] : '';
	$slug    = isset( $item['slug'] ) ? sanitize_key( (string) $item['slug'] ) : (string) $def['slug'];
	$allowed = array( 'spotify', 'apple', 'youtube' );
...
$raw_meta  = isset( $attributes['metaItems'] ) && is_array( $attributes['metaItems'] ) ? $attributes['metaItems'] : array();
$meta_rows = array();
foreach ( $raw_meta as $index => $item ) {
	if ( ! is_array( $item ) ) {
		continue;
	}
	$def   = isset( $default_meta[ $index ] ) ? $default_meta[ $index ] : array( 'label' => '', 'value' => '' );
	$label = isset( $item['label'] ) ? trim( (string) $item['label'] ) : (string) $def['label'];
...
if ( empty( $meta_rows ) ) {
	$meta_rows = $default_meta;
}

/**
 * Resolve a user-entered URL for nav/platform links.
 *
 * @param string $raw Raw URL or path.
 * @return string URL string or empty.
 */
$mwm_hero_05_resolve_url = static function ( $raw ) {
	$raw = trim( (string) $raw );
	if ( '' === $raw ) {
		return '';
	}
...
$wrapper_attributes = get_block_wrapper_attributes(
	array(
		'class' => 'mwm-hero-05 relative w-full overflow-hidden bg-protagonista',
	)
);
?>
...
<section data-light <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> style="padding-top: calc(var(--header-height, 68px));">
	<div class="mwm-max-1">
		<div class="mwm-hero-05__shell flex min-h-[620px] w-full flex-col gap-[60px] pb-[35px] pt-3 lg:min-h-[768px]">
			<?php if ( $show_nav && ! empty( $nav_items ) ) : ?>
				<?php
				$nav_count = count( $nav_items );
				?>
				<nav class="mwm-hero-05__nav flex flex-wrap items-center gap-3" aria-label="<?php esc_attr_e( 'Migas de pan', 'zenyx' ); ?>">
					<?php foreach ( $nav_items as $nav_index => $nav_item ) : ?>
						<?php
						$n_label = $nav_item['label'];
						$n_href  = $mwm_hero_05_resolve_url( $nav_item['url'] ?? '' );
						$is_last = ( $nav_index === $nav_count - 1 );
						$is_link = ! $is_last && '' !== $n_href;
						?>
						<?php if ( $is_link ) : ?>
							<a class="mwm-hero-05__nav-link text-sm text-neutral-light no-underline transition-colors hover:text-acento-hover hover:no-underline" href="<?php echo esc_url( $n_href ); ?>">
								<?php echo esc_html( $n_label ); ?>
							</a>
						<?php else : ?>
							<span class="mwm-hero-05__nav-text text-sm text-neutral-light"<?php echo $is_last ? ' aria-current="page"' : ''; ?>><?php echo esc_html( $n_label ); ?></span>
						<?php endif; ?>
					<?php endforeach; ?>
				</nav>
			<?php endif; ?>

			<div class="mwm-hero-05__row grid grid-cols-1 min-h-0 flex-1 items-stretch gap-6 lg:grid-cols-2 lg:items-end lg:gap-6">
				<div class="mwm-hero-05__left flex min-h-0 min-w-0 flex-1 flex-col justify-between gap-6 self-stretch lg:gap-6">
					<div class="mwm-hero-05__copy flex flex-col justify-end gap-6">
						<?php if ( '' !== trim( wp_strip_all_tags( $heading ) ) ) : ?>
							<h1 class="mwm-hero-05__title max-w-[636px] text-[2rem] font-heading leading-[1.2] text-white md:text-5xl">
								<?php echo wp_kses_post( $heading ); ?>
							</h1>
						<?php endif; ?>

						<?php if ( '' !== trim( wp_strip_all_tags( $lead ) ) ) : ?>
							<p class="mwm-hero-05__lead max-w-[636px] text-xl leading-[1.2] text-neutral-light">
					...
					<?php if ( ! empty( $platforms ) ) : ?>
						<div class="mwm-hero-05__platforms flex flex-wrap items-start gap-6">
							<?php foreach ( $platforms as $p ) : ?>
								<?php
								$p_url   = $mwm_hero_05_resolve_url( $p['url'] ?? '' );
								$p_label = $p['label'];
								$p_slug  = $p['slug'];
								$tag     = '' !== $p_url ? 'a' : 'div';
								?>
					...
					<?php if ( ! empty( $meta_rows ) ) : ?>
						<div class="mwm-hero-05__meta flex flex-wrap gap-6">
							<?php foreach ( $meta_rows as $row ) : ?>
								<div class="mwm-hero-05__meta-col flex min-w-0 flex-1 flex-col gap-3">
									<?php if ( '' !== trim( wp_strip_all_tags( $row['label'] ) ) ) : ?>
										<p class="mwm-hero-05__meta-label text-base text-white"><?php echo wp_kses_post( $row['label'] ); ?></p>
									<?php endif; ?>
									<?php if ( '' !== trim( wp_strip_all_tags( $row['value'] ) ) ) : ?>
										<p class="mwm-hero-05__meta-value text-base text-neutral-light"><?php echo wp_kses_post( $row['value'] ); ?></p>
									<?php endif; ?>
								</div>
							<?php endforeach; ?>
						</div>
				...
				<div class="mwm-hero-05__right flex w-full shrink-0 flex-col items-end justify-end gap-10 lg:flex-1">
					<?php if ( '' !== trim( wp_strip_all_tags( $aside_highlight ) ) ) : ?>
						<p class="mwm-hero-05__aside-highlight w-full max-w-[416px] text-xl leading-[1.2] text-acento">
							<?php echo wp_kses_post( $aside_highlight ); ?>
						</p>
					<?php endif; ?>

					<div class="mwm-hero-05__media-wrap relative w-full max-w-[416px] overflow-hidden">
						<?php if ( '' !== trim( $image_url ) ) : ?>
							<img
								class="mwm-hero-05__media h-full w-full object-cover"
								src="<?php echo esc_url( $image_url ); ?>"
								alt="<?php echo esc_attr( $image_alt ); ?>"
							/>
						<?php else : ?>
							<div class="mwm-hero-05__media-placeholder h-full w-full" aria-hidden="true"></div>
						<?php endif; ?>
					</div>
				</div>
			</div>
