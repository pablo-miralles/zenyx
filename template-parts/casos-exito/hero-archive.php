<?php
/**
 * Casos de exito archive hero (Customizer-driven).
 *
 * @package zenyx
 */
...
$title = get_option( 'mwm_casos_archive_hero_title' );
$title = is_string( $title ) ? trim( $title ) : '';
$title = '' !== $title ? $title : __( 'Casos de exito', THEME_TEXT_DOMAIN );

$lead = get_option( 'mwm_casos_archive_hero_lead' );
$lead = is_string( $lead ) ? trim( $lead ) : '';
$lead = '' !== $lead ? $lead : __( 'Historias reales de crecimiento, procesos y resultados en agencias.', THEME_TEXT_DOMAIN );

$subline = get_option( 'mwm_casos_archive_hero_subline' );
$subline = is_string( $subline ) ? trim( $subline ) : '';
$subline = '' !== $subline ? $subline : __( 'Descubre como otras agencias desbloquearon su siguiente nivel.', THEME_TEXT_DOMAIN );

$cta_text = get_option( 'mwm_casos_archive_hero_cta_text' );
$cta_text = is_string( $cta_text ) ? trim( $cta_text ) : '';
$cta_text = '' !== $cta_text ? $cta_text : __( 'Ver casos', THEME_TEXT_DOMAIN );

$cta_url = get_option( 'mwm_casos_archive_hero_cta_url' );
$cta_url = is_string( $cta_url ) ? trim( $cta_url ) : '#casos';
if ( '' === $cta_url ) {
	$cta_url = '#casos';
}
...
?>
<section data-dark class="overflow-hidden bg-neutral-light pt-header pb-10 lg:min-h-[700px] lg:pb-9" aria-labelledby="mwm-casos-hero-title" >
	<div class="mwm-max-1">
		<div class="relative">
			<?php if ( '' !== $gradient_src ) : ?>
				<img
			...
			<div class="relative z-2 flex min-h-[min(700px,90svh)] flex-col justify-between gap-10 pt-3 pb-8 lg:gap-14 lg:pb-0">
				<nav class="w-full shrink-0" aria-label="<?php esc_attr_e( 'Migas de pan', THEME_TEXT_DOMAIN ); ?>">
					<ol class="flex flex-wrap items-center gap-3 text-sm text-protagonista">
						...
						<li class="font-medium" aria-current="page">
							<?php esc_html_e( 'Casos de exito', THEME_TEXT_DOMAIN ); ?>
						</li>
					</ol>
				</nav>
		
				<div class="flex w-full flex-col items-center gap-6 text-center lg:max-w-[636px] lg:mx-auto">
					<h1 id="mwm-casos-hero-title" class="font-heading text-display-l text-protagonista w-full leading-[1.2]">
